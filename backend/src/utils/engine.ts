...
// ------------------------------------------------------------------
export const correlateAlertToIncident = async (alertId: string, source: string, severity: string) => {
...
// ------------------------------------------------------------------
export const checkBehavioralAnomaly = async (source: string, eventType: string, details: any): Promise<{ isAnomaly: boolean; reason?: string }> => {
    // --- Check 1: New Processes (Existing) ---
    if (eventType === 'PROCESS_SPAWN') {
        const processName = details.process;
        if (processName) {
            let baseline = await prisma.behavioralBaseline.findUnique({
            ...
            if (!baseline) {
                ...
                const knownProcesses = baseline.value as string[];
    ...
    let volumeBaseline = await prisma.behavioralBaseline.findUnique({
    ...
    if (!volumeBaseline) {
        ...
        const stats = volumeBaseline.value as { avg: number; threshold: number };
        ...
        await prisma.behavioralBaseline.update({
